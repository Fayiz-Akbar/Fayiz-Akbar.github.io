...
const ARROW_CLASSES =
  "relative flex items-center justify-center rounded-full border border-slate-200 bg-white text-slate-700 cursor-pointer shrink-0 z-30 outline-none shadow-md hover:border-blue-200 hover:text-blue-600 hover:shadow-lg active:scale-95 transition-all duration-300";
  ...
  return (
    <section className="flex flex-col items-center w-full py-4 lg:py-8 px-4 md:px-8 relative z-20">
      <div className="flex items-center justify-center w-full max-w-[90rem]">
        <div ref={containerRef} className="fan-layout flex relative justify-center items-center w-full max-w-[80rem] h-[30rem] sm:h-[40rem]">
          {cards.map((card, index) => {
            ...
            return card.linkUrl ? (
              <a key={index} href={card.linkUrl} target={card.linkUrl.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className="fan-card block cursor-pointer absolute w-48 h-72 sm:w-64 sm:h-96">{image}</a>
            ) : (
              <div key={index} className="fan-card absolute w-48 h-72 sm:w-64 sm:h-96">{image}</div>
            );
          })}
        </div>
      </div>
      ...
      {needsPagination && (
        <div className="flex items-center justify-center gap-4 mt-4 md:mt-6 z-30">
          ...
          <div className="flex items-center gap-2.5 mx-4">
            {cards.map((_, i) => (
              <span key={i} className={`w-2.5 h-2.5 rounded-full transition-all duration-300 ${i === centerIndex ? "bg-blue-600 scale-[1.3] shadow-sm" : "bg-slate-300 hover:bg-slate-400"}`} />
            ))}
          </div>
